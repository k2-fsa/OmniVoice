...
def write_summary_csv(path: Path, systems: dict) -> None:
    fields = [
        ...
        "median_latency_ms",
        "mean_normalized_cer",
        "canonical_wilson_low",
        "canonical_wilson_high",
    ]
    with path.open("w", encoding="utf-8", newline="") as stream:
        ...
        for name, values in systems.items():
            writer.writerow(
                {
                    "system": name,
                    **{key: values.get(key) for key in fields if key != "system"},
                }
            )
...
def fraction(value: list[int]) -> str:
    return (
        f"{value[0]}/{value[1]} = {100 * value[0] / value[1]:.1f}%"
        if value[1]
        else "0/0"
    )


def write_value_gate(
    path: Path,
    manifest: dict,
    audit: dict,
    summary: dict,
    ...
    lines = [
        ...
        f"- Historical 115/160: {audit['historical_115_of_160']}",
        "",
        "## Text baselines",
        "",
    ...
    for name in ("current_rule", "num2words_vi", "vietnormalizer", "soe_vinorm"):
        value = systems[name]
        lines.append(
            f"| {name} | {value['attempted']}/{value['records']} | "
            f"{fraction(value['canonical_preferred_pair'])} | {fraction(value['acceptable_pair'])} | "
            f"{value['runtime_error']} runtime errors |"
        )
    lines += [
        "",
        f"Best existing automatic baseline by predeclared canonical preferred accuracy: **{best_existing}**.",
