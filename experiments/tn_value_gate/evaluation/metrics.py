...
def mcnemar_exact(left: list[bool], right: list[bool]) -> dict:
    ...
    if not discordant:
        ...
        tail = (
            sum(math.comb(discordant, k) for k in range(min(left_only, right_only) + 1))
            / 2**discordant
        )
...
def paired_bootstrap(
    left: list[bool], right: list[bool], seed: int = 20260722, samples: int = 10000
) -> dict:
    if not left:
    ...
    diffs.sort()
    return {
        "difference": sum(right) / size - sum(left) / size,
