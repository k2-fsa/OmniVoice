...
def main() -> None:
    ...
    for row in selected:
        for condition, field in (
            ("RAW", "original_text"),
            ("BEST_EXISTING", "best_existing_text"),
            ("GOLD", "preferred_gold"),
        ):
            conditions.append(
                {
    ...
    with (
        blind_path.open("w", encoding="utf-8", newline="") as blind,
        key_path.open("w", encoding="utf-8", newline="") as key,
        rating_path.open("w", encoding="utf-8", newline="") as rating,
    ):
        blind_writer = csv.DictWriter(
            blind,
            fieldnames=["blind_sample_id", "source_case_id", "role", "audio_path"],
        )
        key_fields = [
            "blind_sample_id",
            "source_case_id",
        ...
        rating_fields = [
            "blind_sample_id",
            "source_case_id",
            "number_reading_correctness",
            "naturalness_1_to_5",
            "intelligibility_1_to_5",
            "perceived_spoken_form",
            "comment",
        ]
        ...
        for index, row in enumerate(conditions, 1):
            ...
            key_writer.writerow(
                {"blind_sample_id": blind_id, **row, "audio_path": audio_path}
            )
