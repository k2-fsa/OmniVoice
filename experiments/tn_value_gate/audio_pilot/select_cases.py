...
def round_robin(values: list[dict], count: int) -> list[dict]:
    ...
    for value in values:
        groups[value["role"]].append(value)
    selected = []
...
def main() -> None:
        ...
        candidates.append(
            {
                ...
                "role": current["role"],
                "best_existing_system": best,
                "best_existing_text": existing["output_text"],
    ...
    if len(selected) < 24:
        ...
        selected += [value for value in candidates if value["id"] not in used][
            : 24 - len(selected)
        ]
    ...
    with args.output.open("w", encoding="utf-8", newline="") as stream:
        ...
        for index, value in enumerate(selected, 1):
            rationale = (
                "contextual/high-risk disagreement"
                if value["role"] in HIGH_RISK
                else "deterministic control"
            )
            writer.writerow(
                {"selection_order": index, **value, "selection_rationale": rationale}
            )
    print(
        json.dumps(
            {
