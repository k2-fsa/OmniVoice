...
def main() -> None:
    ...
    if args.method == "vietnormalizer":
        ...
        metadata = {"version": __version__, "onnxruntime": onnxruntime.__version__,
                    "model": "vinhdq842/soe-vinorm@cb9705b", "device": "CPU"}
    for line in sys.stdin:
        request = json.loads(line)
        started = time.perf_counter()
        try:
            output = normalizer.normalize(request["text"])
            response = {"id": request["id"], "output_text": output, "supported": True,
                        "status": "success", "error_type": None, "error_message": None,
                        "latency_ms": (time.perf_counter() - started) * 1000, "metadata": metadata}
        except Exception as error:
            response = {"id": request["id"], "output_text": "", "supported": False,
                        "status": "runtime_error", "error_type": type(error).__name__,
